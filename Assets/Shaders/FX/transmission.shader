...
Shader "Omega/FX/transmission" {
	Properties{
		...
		[HideInInspector]_ColorfulMetal("彩色金属补偿", Range(0.0, 2.0)) = 1
		...
		SubShader
	{
		...
		Pass
		{
			...
			CustomVertexOutputForward Customvert(CustomVertexInput v)
			{
			...
			half4 Customfrag(CustomVertexOutputForward i) : COLOR
			{
				//input tex
				...
				float3 finalColor = saturate(_DissolveColor * (NoV + _ColorFactor));
				//float3 finalColor = lerp(_DissolveColor * NoV/* + _ColorFactor*/, grabTex.rgb, _Transparency);
				//destColortex.rgb += _DissolveEdgeColor.rgb*edge;
				//finalColor = lerp(fixed3(alpha), finalColor, _DissolveThreshold);
